...
class PosicionGlobalController < ApplicationController
  ...
  private
    def factura_por_tipo tipo
      ...
      case tipo
        when "compras"
          Factura.find :all, :order => 'facturas.fecha DESC, facturas.codigo DESC', :include => ["albaranes"], :conditions => ["albaranes.proveedor_id IS NOT NULL AND " + filtro_fecha]
        when "ventas"
          Factura.find :all, :order => 'facturas.fecha DESC, facturas.codigo DESC', :include => ["albaranes"], :conditions => ["albaranes.cliente_id IS NOT NULL AND " + filtro_fecha ]
        when "servicios"
