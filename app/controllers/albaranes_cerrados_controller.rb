...
class AlbaranesCerradosController < ApplicationController
  ...
  def listado
    ...
    @albaranes = Albaran.paginate(order: 'fecha DESC, codigo DESC', conditions: condicion, page: (params[:format] == 'xls' ? nil : params[:page]), :per_page (params[:format_xls_count] || Configuracion.valor('PAGINADO') ))
  ...
  def generar_factura
    ...
    if @albaran && !@albaran.deposito && @albaran.cerrado && @albaran.factura.nil?
      ...
      if @factura.errors.empty?
        render 'factura/formulario', locals: { proveedor: (@albaran.proveedor.nombre if @albaran && @albaran.proveedor_id) }
      else
